...
- `generar_informe.py` — arma el Excel final con formato profesional.
- `main.py` — corre todo el flujo de punta a punta.

## Uso

1. Instalar dependencias (ya vienen en este entorno; en el tuyo):
   ```bash
   pip install pandas openpyxl
   ```

2. Poner los 3 archivos que bajás de Qlik en la carpeta `data/`, con
...
1. Escribí la función en `cruces.py` (mirá `cruce_riesgo_cartera` como
   ejemplo de cómo cruzar dos tablas por nombre de cliente).
2. Llamala desde `main.py`.
3. Agregá una hoja nueva en `generar_informe.py` con `_write_table()`.

## Conectar esto con la automatización de Qlik

Si en el futuro el script de extracción de Qlik (`qlik-extractor`, el
proyecto en Node.js que ya armamos) genera los CSV/Excel automáticamente,
podés encadenar este script después, apuntando `config.py` a esos
archivos generados — así todo el flujo (extraer de Qlik → cruzar →
informe) corre sin intervención manual.
